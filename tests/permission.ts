...
test('registers a custom role', t => {
    ...
    permissions.registerRole(OtherRole);
    ...
    t.is(Raider.getId(), LoveRaider.getId());
});

test('get a role from an Id', t => {
    t.is(permissions.getById(LoveRaider.getId()), LoveRaider);
    t.not(permissions.getById(LoveRaider.getId()), permissions.get('Moderator'));
...
test('has a type', t => {
    const Moderator = permissions.get('Moderator');
    t.is('internal', Moderator.getType());
    t.not('custom', Moderator.getType());
});
